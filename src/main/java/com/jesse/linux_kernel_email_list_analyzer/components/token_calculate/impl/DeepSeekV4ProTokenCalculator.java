...
/**
 * DeepSeek-V4-Pro 模型 Token 资费计算器。
 *
 * <pre>
 * DeepSeek-V4-Pro Token 费计算公式：
 * 百万输入词元 (缓存命中)   * 0.025  +
 * 百万输入词元 (缓存未命中) * 3.00   +
 * 百万输出词源              * 6.00
 * </pre>
 */
@NoArgsConstructor
public class DeepSeekV4ProTokenCalculator implements ModelTokenCalculator
{
    /** 命中缓存的输入提示词每百万 Token 的价格 */
    private static final
    BigDecimal PROMPT_CACHE_HIT_PRICE = new BigDecimal("0.025");

    /** 未命中缓存的输入提示词每百万 Token 的价格 */
    private static final
    BigDecimal PROMPT_CACHE_MISS_PRICE = new BigDecimal("3.00");

    /** 输出提示词每百万 Token 的价格 */
    private static final
    BigDecimal COMPLETION_PRICE = new BigDecimal("6.00");
